...
inline DirectX::XMMATRIX AssimpToDxMatrix(const aiMatrix4x4& m)
{
...
void CubeRenderComponent::VInit()
{
    ...
    auto node = std::make_shared<ShaderMeshNode>(m_pOwner->GetId(), std::string("Cube render node"),
...
TextureCubeRenderComponent::TextureCubeRenderComponent(std::shared_ptr<Actor> owner, Scene* scene, float width,
                                                       float height, float depth) : RenderComponent(
    std::move(owner), scene)
{
    w = width;
    h = height;
    d = depth;
...
void WireframeCubeRenderComponent::VInit()
{
    ...
    Graphics::GetDevice()->CreateInputLayout(layout, 1, vsBlob->GetBufferPointer(), vsBlob->GetBufferSize(),
                                             &inputLayout);
